...
func dataToTargetEpisode(storyData : [BlockOfDayEpisodeForJson], target : inout [String : BlockOfDayEpisode]){
    ...
    for ex in storyData{
        let storyId = ex.id
        let story = BlockOfDayEpisode(chats: ex.chats, choices: ex.choices, choiceSkip: ex.choiceSkip, isGod: ex.isGod, backGroundMusic: ex.backGroundMusic, currentRoute: ex.currentRoute)
