...
//        mapImage.image = UIImage(named: "\(name)map")
    ...
    func getIndexOf(button: MapPlaceButton) -> Int? {
  ...
  private func updateCurrentButton(index: Int) {
    ...
    let spotsName : [Spots] = [
        Spots(number: "①", name: "돈화문", images: ["donhwamun1", "donhwamun2", "donhwamun3", "donhwamun4", "donhwamun5", "donhwamun6", "donhwamun7", "donhwamun8", "donhwamun9"], description: "돈화문은 창덕궁의 정문으로, 보물 제383호로 지정되어 있다.\n1412년(태종 12)에 건립된 돈화문은 궁궐 대문 중에서 가장 큰 규모이자 현존하는 궁궐 대문 중에서 가장 오래된 목조 건물이다.\n임진왜란 때 소실되어 광해군이 즉위한 뒤 1609년에 다시 재건되었다.\n\n돈화문의 ‘돈화(敦化)’는 ‘임금이 큰 덕을 베풀어 백성들을 돈독하게 교화 한다’는 뜻이다.\n이는 유학 경전 중 하나인 [중용(中庸)]에 나오는 표현을 확장시킨 것이다."),
         ...
         Spots(number: "⑤", name: "선정전", images: ["seonjeongjeon1", "seonjeongjeon2", "seonjeongjeon3", "seonjeongjeon4", "seonjeongjeon5", "seonjeongjeon6"], description: "선정전은 임금이 고위직 신하들과 함께 일상 업무를 보던 공식 집무실인 편전으로, 보물 제814호로 지정되어 있다.\n아침의 조정회의, 업무 보고, 경연 등 각종 회의가 이곳에서 열렸다.\n‘정치는 베풀어야 한다’는 뜻을 가진 선정전은 1461년(세조 7)에 바뀐 이름으로, 창건 당시에는 조계청이라 불리었다.\n임진왜란과 인조반정 등의 화재로 소실되었다가 1647년(인조 25)에 인경궁을 헐어 그 재목으로 재건하였다.\n현재 궁궐에 남아 있는 유일한 청기와 건물이다."),
      
         Spots(number: "⑥", name: "희정당", images: ["huijeongdang1", "huijeongdang2", "huijeongdang3", "huijeongdang4", "huijeongdang5", "huijeongdang6", "huijeongdang7", "huijeongdang8", "huijeongdang9", "huijeongdang10"], description: "본래 침전으로 사용하다가, 조선 후기부터 왕의 집무실로 사용된 희정당은 보물 제815호로 지정되어 있다.\n1496년(연산 2)에 숭문당이라는 건물이 소실되었고, 다시 재건하면서 이름을 희정당이라고 부르게 되었다.\n이후에도 몇 차례 화재로 다시 지었는데 현재 건물은 일제강점기 때 1917년에 불 탄 것을 경복궁의 침전인 강녕전을 헐어서 1920년에 지은 것이다.\n구한말 왕의 집무실과 외국 사신들을 접대하는 곳으로 사용하면서 희정당 내부에는 카펫, 유리 창문, 샹들리에 등이 설치하여 서양식으로 변화하였다."),
         ...
         Spots(number: "⑧", name: "낙선재", images: ["nakseonjae1", "nakseonjae2", "nakseonjae3", "nakseonjae4", "nakseonjae5", "nakseonjae6"], description: "현종의 검소한 면모가 잘 느껴지는 낙선재는 보물 제1764호로 지정되어 있다.\n1847년 헌종이 왕비와 대왕대비를 위해 마련한 곳으로, 조선 왕실의 권위를 확립하고 자신의 개혁 의지를 실천하기 위한 장소로 사용했다.\n낙선재는 궁궐의 침전 형식이 응용되면서 다른 곳에서는 쉽게 찾아볼 수 없는 다양한 문양의 장식들을 볼 수 있다."),
      
         Spots(number: "⑨", name: "부용지", images: ["buyongji1", "buyongji2", "buyongji3", "buyongji4", "buyongji5", "buyongji6", "buyongji7", "buyongji8"], description: "후원의 첫 번째 중심 정원인 부용지는 사각형 모양의 연못이다.\n부용지를 중심으로 휴식과 학문적 용도로 쓰인 아름다운 건물들이 모여있다.\n연꽃이 활짝 핀 모양의 부용정은 연못에 핀 한 송이의 꽃 형상으로 보물 제1763호로 지정되어 있다.\n높은 곳에서 연못을 내려다보는 주합루는 학문을 연마하던 곳으로 보물 제1768호로 지정되어 있다.\n건물 하나하나 모두 각각 특색있고 아름답지만, 서로 어우러지면서 풍경이 되는 것이 절묘한 경관이다."),
      
         Spots(number: "⑩", name: "불로문", images: ["bullomun1"], description: "연경당 입구에 서 있는 불로문은 이 문을 통과하면 늙지 않는다는 설이 있다. 이는 왕의 무병 장수를 기원하기 위해 만들어졌다."),
         ...
         Spots(number: "⑫", name: "연경당", images: ["yeongyeongdang1", "yeongyeongdang2", "yeongyeongdang3", "yeongyeongdang4", "yeongyeongdang5", "yeongyeongdang6", "yeongyeongdang7", "yeongyeongdang8", "yeongyeongdang9"], description: "사대부의 살림집을 본뜬 조선 후기 접견실인 연경당은 보물 제1770호로 지정되어 있다.\n연경당의 ‘연경(演慶)’은 경사가 널리 퍼진다는 뜻이다.\n연경당은 효명세자가 순조와 순원왕후를 위한 잔치를 열고자 지은 집니다. 창건 직후 이곳에서는 효명세자가 신하를 접견하거나 연회를 펼쳤다. 헌종 이후 효명세자의 어진과 모훈을 보관하는 곳으로 사용하다가 고종 때에 이르러 외국 공사를 접견하고 연회를 베푸는 중요한 정치 공간으로 이용하였다.\n유교적 철학과 궁전의 조영법식이 어우러져 건축된 연경당은 조선시대 궁궐 내 사대부 건축으로서 그 가치가 매우 뛰어나다."),
      
         Spots(number: "⑬", name: "폄우사", images: ["pyeomusa1"], description: "반월지 연못의 서쪽 언덕 위에 위치한 길쭉한 맞배지붕 형태를 가진 폄우사는 본래 부속채가 딸린 ‘ㄱ’자 모양이었으나 현재는 부속채가 없어져 단출한 모습을 갖추게 되었다."),
      
         Spots(number: "⑭", name: "존덕정", images: ["jondeokjeong1"], description: "반월지 연못을 중심으로 겹지붕의 육각형 정자인 존덕정은 조선 건축의 색다른 구상과 재주를 보여주는 중요한 건물이다."),
      
         Spots(number: "⑮", name: "옥류천", images: ["ongnyucheon1", "ongnyucheon2"], description: "옥류천은 후원 북쪽 가장 깊은 골짜기에 흐른다.\n1636년(인조 14)에 거대한 바위인 소요암을 깎아내고 그 위에 홈을 파서 작은 폭포를 만들었다.\n바위에 새겨진 '玉流川' 세 글자는 인조의 친필이다.")]
}
