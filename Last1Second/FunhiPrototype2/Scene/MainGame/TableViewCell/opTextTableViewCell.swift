...
class OpTextTableViewCell: UITableViewCell {
  ...
  func configure(name: GameCharacterID, chat: String, normalProfile: String, mainProfile: CharacterFace, isLocked: Bool, godchat: Bool) {
    switch isLocked {
    case true:
    ...
    let isProfileHiddle = mainProfile == .none
    profileImage.isHidden = isProfileHiddle
    profileNickname.isHidden = isProfileHiddle
