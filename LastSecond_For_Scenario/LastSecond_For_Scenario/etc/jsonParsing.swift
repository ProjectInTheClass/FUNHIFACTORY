...
func dataToTargetEpisode(storyData: [BlockOfDayEpisodeForJson], target: inout [String: BlockOfDayEpisode]) {
  ...
  for ex in storyData {
    let storyId = ex.id
    let story = BlockOfDayEpisode(id: ex.id, chats: ex.chats, choices: ex.choices, choiceSkip: ex.choiceSkip, isGodChat: ex.isGodChat, backGroundMusic: ex.backGroundMusic, currentRoute: ex.currentRoute)
